/**
 * Admin Listings - Returns listings for Ops Review Queue.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let query = supabase
      .from('listings')
      .select(`
        id, seller_id, identifier, title, description, status,
        reserve_price, starting_price, current_bid, image_urls,
    ...
    if (status) {
      ...
      query = query.in('status', ['pending_review', 'draft', 'approved'])
    }
