/**
 * Admin Auctions - Returns auctions for monitor.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let query = supabase
      .from('auctions')
      .select(`
        id, listing_id, status, start_time, end_time,
        reserve_price, current_bid, paused, extended_minutes,
    ...
    const auctions = (rows ?? []).map((r: Record<string, unknown>) => ({
      id: r.id,
      ...
      extended_minutes: r.extended_minutes ?? 0,
